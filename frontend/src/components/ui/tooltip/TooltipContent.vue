...
const props = withDefaults(defineProps<TooltipContentProps & { class?: HTMLAttributes["class"], arrowClass?: HtmlHTMLAttributes['class'] }>(), {
  sideOffset: 4,
})
...
<template>
  <TooltipPortal>
    <TooltipContent data-slot="tooltip-content" v-bind="{ ...forwarded, ...$attrs }"
      ...
      <TooltipArrow
        :class="cn('bg-foreground fill-foreground z-50 size-2.5 translate-y-[calc(-50%_-_2px)] rotate-45 rounded-[2px]', props.arrowClass)" />
    </TooltipContent>
  </TooltipPortal>
</template>
